...
function initReviewsSlider() {
	if (!document.querySelector("[data-fls-reviews]")) return;
	new Swiper("[data-fls-reviews]", {
		modules: [Pagination],
		observer: true,
		observeParents: true,
		slidesPerView: 1,
...
function headerDropdownInit() {
	function closeAll() {
		document.querySelectorAll(".menu__link--dropdown[aria-expanded=\"true\"]").forEach((btn) => {
			btn.setAttribute("aria-expanded", "false");
		});
	}
	document.addEventListener("click", function(e) {
		const trigger = e.target.closest(".menu__link--dropdown");
		if (!trigger) {
			closeAll();
			return;
		}
	...
	document.addEventListener("keydown", function(e) {
		if (e.key === "Escape") {
			closeAll();
			const openTrigger = document.querySelector(".menu__link--dropdown[aria-expanded=\"true\"]");
			if (openTrigger) openTrigger.focus();
		}
	});
}
document.querySelector(".menu__link--dropdown") && window.addEventListener("load", headerDropdownInit);
//#endregion
