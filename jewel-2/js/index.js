...
function menuInit() {
	document.addEventListener("click", function(e) {
		...
		if (bodyLockStatus && menuButton) {
			...
			if (!isOpen) closeMenuDropdowns();
		}
		const subItem = e.target.closest(".menu__item_sub");
		const isToggleTrigger = e.target.closest(".menu__arrow") || e.target.closest(".menu__item_sub > .menu__link");
		if (subItem && isToggleTrigger) {
			if (e.target.closest(".menu__item_sub > .menu__link")) e.preventDefault();
			const isActive = subItem.classList.contains("--active");
			closeMenuDropdowns();
			if (!isActive) {
				subItem.classList.add("--active");
				const arrow = subItem.querySelector(".menu__arrow");
				if (arrow) arrow.setAttribute("aria-expanded", "true");
			}
		} else if (!subItem) closeMenuDropdowns();
	});
	document.addEventListener("keydown", function(e) {
		if (e.code === "Escape") closeMenuDropdowns(true);
	});
	function closeMenuDropdowns(returnFocus = false) {
		document.querySelectorAll(".menu__item_sub.--active").forEach((item) => {
...
function addToCart() {
	...
	function addToCartAction(e) {
		...
		if (targetElement.closest("[data-fls-addtocart-button]")) {
			...
			const addToCartProduct = targetElement.closest("[data-fls-addtocart-button]").closest("[data-fls-addtocart-product]");
			if (addToCartProduct) {
				...
				addToCartImage && addToCartImageFly(addToCartImage, addToCart, flyImgSpeed);
				setTimeout(() => {
					addToCart.innerHTML = +addToCart.innerHTML + addToCartQuantity;
				}, addToCartImage ? flyImgSpeed : 0);
			} else addToCart.innerHTML = +addToCart.innerHTML + 1;
		}
	}
