...
function Parallax$1({ swiper, extendParams, on }) {
			...
			slider.slides.forEach((slide) => {
				const topImage = `
							<div class="slide-hero__top-image">
								<img src="${slide.querySelector(".slide-hero__image").getAttribute("src")}" alt="Image">
							</div>
						`;
				slide.querySelector(".slide-hero__content").insertAdjacentHTML("beforeend", topImage);
			});
		} }
	});
}
	...
	*/
	...
	LightGallery.prototype.getItems = function() {
		...
		if (!this.settings.dynamic) {
			...
			else if (this.settings.selector) if (typeof this.settings.selector === "string") if (this.settings.selectWithin) this.items = $LG(this.settings.selectWithin).find(this.settings.selector).get();
